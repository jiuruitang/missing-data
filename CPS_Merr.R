...
# a1 <- 30; a2 <- 41; a3 <- 50
a1 <- 60; a2 <- 130; a3 <- 190
b1 <- 1000-a1; b2 <- 1000-a2; b3 <- 1000-a3

nc_adj$voted_obs <- nc_adj$voted

n_educ1_voted <- 
  dim(nc_adj[which(nc_adj$unit == 0 & nc_adj$Rv == 0 & nc_adj$voted == 1 & nc_adj$educ == 1),])[1]
nc_adj[which(nc_adj$unit == 0 & nc_adj$Rv == 0 & nc_adj$voted == 1 & nc_adj$educ == 1),]$voted <- 
  rbinom(n_educ1_voted,1,prob = rep(0.81,n_educ1_voted))
n_educ2_voted <- 
  dim(nc_adj[which(nc_adj$unit == 0 & nc_adj$Rv == 0 & nc_adj$voted == 1 & nc_adj$educ == 2),])[1]
nc_adj[which(nc_adj$unit == 0 & nc_adj$Rv == 0 & nc_adj$voted == 1 & nc_adj$educ == 2),]$voted <- 
  rbinom(n_educ2_voted,1,prob = rep(0.86,n_educ2_voted))
n_educ3_voted <- 
  dim(nc_adj[which(nc_adj$unit == 0 & nc_adj$Rv == 0 & nc_adj$voted == 1 & nc_adj$educ == 3),])[1]
nc_adj[which(nc_adj$unit == 0 & nc_adj$Rv == 0 & nc_adj$voted == 1 & nc_adj$educ == 3),]$voted <- 
  rbinom(n_educ3_voted,1,prob = rep(0.89,n_educ3_voted))

nc_adj$voted[which(nc_adj$voted_obs == 0)] <- 0
...
for (i in 1:niter){
  print(i)
  {
    ...
    # measurement error
    full_obs <- nc_adj[which(nc_adj$Rv == 0 & nc_adj$unit == 0),]
    m_vote_full <- glm(voted ~ as.factor(sex) + age + as.factor(race) + as.factor(educ)
                  + as.factor(sex)*as.factor(race) + as.factor(sex)*age + as.factor(sex)*as.factor(educ),
                  data = full_obs,family=binomial())
    vote_mat_full <- model.matrix(m_vote_full)
    
    # Re
    # r_eth_full <- glm(Re~as.factor(sex) + as.factor(educ) + as.factor(age) + as.factor(voted), 
    #              data = full_obs,family=binomial())
    # ethR_mat_full <- model.matrix(r_eth_full)
    # gamma_e_full <- rmvnorm(1,mean = c(t(coef(r_eth_full))),sigma = vcov(r_eth_full))
    # 
    # ethR_mat_full[,ncol(ethR_mat_full)] <- 0
    # Re_vote0 <- logit_inv(ethR_mat_full%*%t(gamma_e_full)) 
    # ethR_mat_full[,ncol(ethR_mat_full)] <- 1
    # Re_vote1 <- logit_inv(ethR_mat_full%*%t(gamma_e_full)) 
    
    # Rc
    # r_educ_full <- glm(Rc~as.factor(sex) + as.factor(race) + as.factor(age) +  as.factor(voted),
    #                             data = full_obs,family=binomial())
    # educR_mat_full <- model.matrix(r_educ_full)
    # gamma_c_full <- rmvnorm(1,mean = c(t(coef(r_educ_full))),sigma = vcov(r_educ_full))
    # 
    # educR_mat_full[,ncol(educR_mat_full)] <- 0
    # Rc_vote0 <- logit_inv(educR_mat_full%*%t(gamma_c_full)) 
    # educR_mat_full[,ncol(educR_mat_full)] <- 1
    # Rc_vote1 <- logit_inv(educR_mat_full%*%t(gamma_c_full)) 
    
    # prior for regression model
    # nu0 <- SS0 <-1
    # phi <- 1
    # phi0 <- solve(diag(length(coef(m_vote_full))))
    # b0 <- rep(0,length(coef(m_vote_full)))
    
    # posterior
    # beta_hat <- coef(m_vote_full)
    # X <- vote_mat_full
    # phin <- t(X)%*%X+phi0
    # bn <- solve(phin)%*%(t(X)%*%X%*%beta_hat + phi0%*%b0)
    # gamma2 <- sum((m_vote_full$residuals)^2)+SS0 + t(beta_hat)%*%t(X)%*%X%*%beta_hat + t(b0)%*%phi0%*%b0-t(bn)%*%phin%*%bn
    # phi <- rgamma(1,(dim(full_obs)[1]+nu0)/2,gamma2/2)
    # alpha_v_full <- rmvnorm(1,mean = bn,sigma = solve(phi*phin))
    
    
    alpha_v_full <- rmvnorm(1,mean = c(t(coef(m_vote_full))),sigma = vcov(m_vote_full))
    
    theta1 <- rbeta(1,sum(full_obs$voted_obs[which(full_obs$voted == 0 & full_obs$educ == 1)] == 1) + a1,
                    sum(full_obs$voted_obs[which(full_obs$voted == 0 & full_obs$educ == 1)] == 0)+b1)
    theta2 <- rbeta(1,sum(full_obs$voted_obs[which(full_obs$voted == 0 & full_obs$educ == 2)] == 1) + a2,
                    sum(full_obs$voted_obs[which(full_obs$voted == 0 & full_obs$educ == 2)] == 0)+b2)
    theta3 <- rbeta(1,sum(full_obs$voted_obs[which(full_obs$voted == 0 & full_obs$educ == 3)] == 1) + a3,
                    sum(full_obs$voted_obs[which(full_obs$voted == 0 & full_obs$educ == 3)] == 0)+b3)
    
    vote_prob_full <- logit_inv(vote_mat_full%*%t(alpha_v_full))
    
    vote1 <- vote_prob_full
    vote0 <- 1-vote_prob_full
    vote0[which(full_obs$educ == 1)] <- (vote0*(ifelse(full_obs$voted_obs == 1,
                                                          theta1,1-theta1)))[which(full_obs$educ == 1)]
    vote0[which(full_obs$educ == 2)] <- (vote0*(ifelse(full_obs$voted_obs == 1,
                                                          theta2,1-theta2)))[which(full_obs$educ == 2)]
    vote0[which(full_obs$educ == 3)] <- (vote0*(ifelse(full_obs$voted_obs == 1,
                                                          theta3,1-theta3)))[which(full_obs$educ == 3)]
    vote1 <- ifelse(full_obs$voted_obs == 1, vote_prob_full,0)
    # vote1_final <- vote1*Rc_vote1*Re_vote1
    # vote0_final <- vote0*Rc_vote0*Re_vote0
    
    nc_adj$voted[which(nc_adj$unit == 0 & nc_adj$Rv == 0)] <- 
      rbinom(dim(vote_prob_full)[1],1,prob = vote1/(vote1+vote0))
    
    if (sum(is.na(nc_adj$voted[which(nc_adj$unit == 0 & nc_adj$Rv == 0)])) > 0){
      break
    }
    # update theta1, theta2, theta3 for overreporting
    # update theta1 and theta0
    # theta1 <- rbeta(1,sum(nc_adj$voted_obs[which(nc_adj$voted == 0 & nc_adj$unit == 0 & nc_adj$educ == 1 & nc_adj$Rv == 0)] == 1) + a1,
    #                 sum(nc_adj$voted_obs[which(nc_adj$voted == 0 & nc_adj$unit == 0 & nc_adj$educ == 1& nc_adj$Rv == 0)] == 0)+b1)
    # theta2 <- rbeta(1,sum(nc_adj$voted_obs[which(nc_adj$voted == 0 & nc_adj$unit == 0 & nc_adj$educ == 2 & nc_adj$Rv == 0)] == 1) + a2,
    #                 sum(nc_adj$voted_obs[which(nc_adj$voted == 0 & nc_adj$unit == 0 & nc_adj$educ == 2 & nc_adj$Rv == 0)] == 0)+b2)
    # theta3 <- rbeta(1,sum(nc_adj$voted_obs[which(nc_adj$voted == 0 & nc_adj$unit == 0 & nc_adj$educ == 3 & nc_adj$Rv == 0)] == 1) + a3,
    #                sum(nc_adj$voted_obs[which(nc_adj$voted == 0 & nc_adj$unit == 0 & nc_adj$educ == 3 & nc_adj$Rv == 0)] == 0)+b3)
    
    # update true vote (voted)
    # vote_prob <- logit_inv(vote_mat%*%t(alpha_v)) # all unit respondents
    # u0_dat <- nc_adj[which(nc_adj$unit == 0),] # all unit respondents
    # vote1 <- vote_prob
    # vote0 <- 1-vote_prob
    # vote0[which(u0_dat$educ == 1 & u0_dat$Rv == 0)] <- (vote0*(ifelse(u0_dat$voted_obs == 1,
    #                                                       theta1,1-theta1)))[which(u0_dat$educ == 1 & u0_dat$Rv == 0)]
    # vote0[which(u0_dat$educ == 2 & u0_dat$Rv == 0)] <- (vote0*(ifelse(u0_dat$voted_obs == 1,
    #                                                       theta2,1-theta2)))[which(u0_dat$educ == 2 & u0_dat$Rv == 0)]
    # vote0[which(u0_dat$educ == 3 & u0_dat$Rv == 0)] <- (vote0*(ifelse(u0_dat$voted_obs == 1,
    #                                                       theta3,1-theta3)))[which(u0_dat$educ == 3 & u0_dat$Rv == 0)]
    # vote1[which(u0_dat$Rv == 0)] <- ifelse(u0_dat$voted_obs == 1, vote_prob,0)[which(u0_dat$Rv == 0)]
    # nc_adj$voted[which(nc_adj$unit == 0 & nc_adj$Rv == 0)] <- rbinom(sum(u0_dat$Rv == 0),1,prob = vote1/(vote1+vote0)) 
    ...
    pr_educ_miss_prob[which(pr_educ_miss_prob == 0)] <- 1e-5
    ...
    for (k in 1:7){
      ...
      voteA_temp <- voteA_mat_mis%*%t(alpha_v_origin)
      voteA_pred <- ifelse(voteA_temp>709,1,logit_inv(voteA_temp))
    ...
    pr_age_miss_prob <-ifelse(is.na(pr_age_miss/rowSums(pr_age_miss)),0,pr_age_miss/rowSums(pr_age_miss))
    pr_age_miss_prob[which(pr_age_miss_prob == 0)] <- 1e-5
...
# 06/23 update: add white cross education 
ans_RC <- matrix(NA,50,12)
ul_RC <- matrix(NA,50,12)
ans_SRC <- matrix(NA,50,24)
ul_SRC <- matrix(NA,50,24)
...
for (i in 1:n){
  ...
  test_dat$id <- as.numeric(test_dat$id)
  mydesign <- svydesign(id = ~id, data = test_dat, weight = ~weight)
  wgt_df_RC <- data.frame(svyby(~as.factor(voted),~as.factor(race)+~as.factor(educ),mydesign,svymean))
  ans_RC[i,] <- wgt_df_RC[,4]
  ul_RC[i,] <- wgt_df_RC[,6]
  wgt_df_SRC <- data.frame(svyby(~as.factor(voted),~as.factor(sex)
                                 +as.factor(race)+~as.factor(educ),mydesign,svymean))
  ans_SRC[i,] <- wgt_df_SRC[,5]
  ul_SRC[i,] <- wgt_df_SRC[,7]
}

colMeans(ans_RC)
u_L_RC <- colMeans(ul_RC^2)
b_L_RC <- apply((scale(ans_RC,scale=FALSE))^2/(50-1),2,sum)
T_L_RC <- (1+1/50)*b_L_RC + u_L_RC
sqrt(T_L_RC)
sqrt(b_L_RC/50)
sqrt(u_L_RC/50)

colMeans(ans_SRC)
u_L_SRC <- colMeans(ul_SRC^2)
b_L_SRC <- apply((scale(ans_SRC,scale=FALSE))^2/(50-1),2,sum)
T_L_SRC <- (1+1/50)*b_L_SRC + u_L_SRC
sqrt(T_L_SRC)
sqrt(b_L_SRC/50)
sqrt(u_L_SRC/50)
...
#####################################################
# 06/02/2022 Update
## Predicted turnout among item and unit nonresponse

ansR <- ulR <- rep(NA,5000)
ansU <- ulU <- rep(NA,5000)
ansF<- ulF <- rep(NA,5000)

for (i in 1:5000){
  test_dat <- as.data.frame(cbind(vote_impute[i,],age_impute[i,],race_impute[i,],sex_impute[i,],educ_impute[i,],
                                  nc_copy$unit,nc_copy$weight,nc_copy$Ra,nc_copy$Rv,
                                  nc_copy$Re,nc_copy$Rc,rownames(nc_copy)))
  names(test_dat) <- c("voted","age","race","sex","educ","unit","weight","Ra","Rv","Re","Rc","id")
  test_dat$voted <- as.numeric(test_dat$voted)
  ...
  test_dat$Rc <- as.numeric(test_dat$Rc)
  test_dat$id <- as.numeric(test_dat$id)
  test_datR <- test_dat[which(test_dat$Rv == 1 & test_dat$unit == 0),]
  test_datU <- test_dat[which(test_dat$unit == 1),]
  test_datF <- test_dat[which(test_dat$Rv == 0 & test_dat$unit == 0),]
  mydesignR <- svydesign(id = ~id, data = test_datR, weight = ~weight)
  mydesignU <- svydesign(id = ~id, data = test_datU, weight = ~weight)
  mydesignF <- svydesign(id = ~id, data = test_datF, weight = ~weight)
  wgt_dfR <- data.frame(svymean(~voted, design=mydesignR))
  ansR[i] <- wgt_dfR[,1]
  ulR[i] <- wgt_dfR[,2]
  wgt_dfU <- data.frame(svymean(~voted, design=mydesignU))
  ansU[i] <- wgt_dfU[,1]
  ulU[i] <- wgt_dfU[,2]
  wgt_dfF <- data.frame(svymean(~voted, design=mydesignF))
  ansF[i] <- wgt_dfF[,1]
  ulF[i] <- wgt_dfF[,2]
}

ansR_noM <- ulR_noM <- rep(NA,5000)
ansU_noM <- ulU_noM <- rep(NA,5000)
ansF_noM<- ulF_noM <- rep(NA,5000)
...
for (i in 1:5000){
  ...
  mydesignF <- svydesign(id = ~id, data = test_datF, weight = ~weight)
  wgt_dfR <- data.frame(svymean(~voted, design=mydesignR))
  ansR_noM[i] <- wgt_dfR[,1]
  ulR_noM[i] <- wgt_dfR[,2]
  wgt_dfU <- data.frame(svymean(~voted, design=mydesignU))
  ansU_noM[i] <- wgt_dfU[,1]
  ulU_noM[i] <- wgt_dfU[,2]
  wgt_dfF <- data.frame(svymean(~voted, design=mydesignF))
  ansF_noM[i] <- wgt_dfF[,1]
  ulF_noM[i] <- wgt_dfF[,2]
}
...
comp_dat <- data.frame(ansR = ansR, ansU = ansU, ansF = ansF, 
                       ansR_noM = ansR_noM, ansU_noM = ansU_noM, ansF_noM = ansF_noM)
box_dat <- data.frame(x = c(ansR,ansR_noM,ansU,ansU_noM,ansF,ansF_noM), sub = c(rep("Item Nonresponse",10000),
                                                                  rep("Unit Nonresponse",10000),rep("Response",10000)), 
                      model = c(rep("MErr",5000),rep("No MErr",5000),rep("MErr",5000),rep("No MErr",5000),rep("MErr",5000),rep("No MErr",5000)))
ggplot(box_dat, aes(x=sub, y=x, fill=model)) + 
  geom_boxplot() + xlab("") + ylab("Turnout")


# 06/13 Examine subgroup turntout among unit nonresponse
n <- 5000
...
for (i in 1:n){
  ...
  test_dat$Rc <- as.numeric(test_dat$Rc)
  test_dat$id <- as.numeric(test_dat$id)
  test_dat$race <- as.factor(test_dat$race)
  # test_datR <- test_dat[which(test_dat$Rv == 1 & test_dat$unit == 0),]
  test_datU <- test_dat[which(test_dat$unit == 1),]
  # test_datF <- test_dat[which(test_dat$Rv == 0 & test_dat$unit == 0),]
  # mydesignR <- svydesign(id = ~id, data = test_datR, weight = ~weight)
  ...
  wgt_tbl <-svymean(~interaction(sex,voted), design=mydesignU)
  wgt_tblR <- svyby(~voted,~as.factor(race),mydesignU,svymean)
  wgt_dfR <- data.frame(wgt_tblR)
  ans_R[i,wgt_dfR[1,1]] <- wgt_dfR[wgt_dfR[1,1],2]
  ans_R[i,wgt_dfR[2,1]] <- wgt_dfR[wgt_dfR[2,1],2]
  ans_R[i,wgt_dfR[3,1]] <- wgt_dfR[wgt_dfR[3,1],2]
  if (!is.na(wgt_dfR[4,1])){
    ans_R[i,wgt_dfR[4,1]] <- wgt_dfR[wgt_dfR[4,1],2]
  }
  # ul_R[i,] <- wgt_dfR[,3]
  wgt_tblA <- svyby(~voted,~as.factor(age),mydesignU,svymean)
  wgt_dfA <- data.frame(wgt_tblA)
  ans_A[i,] <- wgt_dfA[,2]
  ul_A[i,] <- wgt_dfA[,3]
  wgt_tblS <- svyby(~as.factor(voted),~as.factor(sex),mydesignU,svymean)
  wgt_dfS <- data.frame(wgt_tblS)
  ans_S[i,] <- wgt_dfS[,3]
  ul_S[i,] <- wgt_dfS[,5]
  wgt_tblC <- svyby(~as.factor(voted),~as.factor(educ),mydesignU,svymean)
  wgt_dfC <- data.frame(wgt_tblC)
  ans_C[i,] <- wgt_dfC[,3]
  ul_C[i,] <- wgt_dfC[,5]
  wgt_df <- data.frame(svymean(~voted, design=mydesignU))
  ans[i] <- wgt_df[,1]
  ul[i] <- wgt_df[,2]
}

nc_full <- nc_copy[which(nc_copy$Rv == 0 & nc_copy$unit == 0),]
prop.table(table(nc_full$voted,nc_full$sex),2)[2,]
colMeans(ans_S)
par(mfrow = c(2,1))
hist(ans_S[,1])
hist(ans_S[,2])

prop.table(table(nc_full$voted,nc_full$race),2)[2,]
colMeans(ans_R,na.rm = T)
par(mfrow = c(2,2))
hist(ans_R[,1])
hist(ans_R[,2])
hist(ans_R[,3])
...
prop.table(table(nc_full$voted,nc_full$age),2)[2,]
colMeans(ans_A)

prop.table(table(nc_full$voted,nc_full$educ),2)[2,]
colMeans(ans_C)


# item and unit respondents for vote
for (i in 1:n){
  test_dat <- as.data.frame(cbind(vote_impute[i,],age_impute[i,],race_impute[i,],sex_impute[i,],educ_impute[i,],
                                  nc_copy$unit,nc_copy$weight,nc_copy$Ra,nc_copy$Rv,
                                  nc_copy$Re,nc_copy$Rc,rownames(nc_copy)))
  ...
  wgt_tbl <-svymean(~interaction(sex,voted), design=mydesignF)
  wgt_tblR <- svyby(~voted,~as.factor(race),mydesignF,svymean)
  wgt_dfR <- data.frame(wgt_tblR)
  ans_R[i,wgt_dfR[1,1]] <- wgt_dfR[wgt_dfR[1,1],2]
  ans_R[i,wgt_dfR[2,1]] <- wgt_dfR[wgt_dfR[2,1],2]
  ans_R[i,wgt_dfR[3,1]] <- wgt_dfR[wgt_dfR[3,1],2]
  ...
  # ul_R[i,] <- wgt_dfR[,3]
  wgt_tblA <- svyby(~voted,~as.factor(age),mydesignF,svymean)
  wgt_dfA <- data.frame(wgt_tblA)
  ans_A[i,] <- wgt_dfA[,2]
  ul_A[i,] <- wgt_dfA[,3]
  wgt_tblS <- svyby(~as.factor(voted),~as.factor(sex),mydesignF,svymean)
  wgt_dfS <- data.frame(wgt_tblS)
  ans_S[i,] <- wgt_dfS[,3]
  ul_S[i,] <- wgt_dfS[,5]
  wgt_tblC <- svyby(~as.factor(voted),~as.factor(educ),mydesignF,svymean)
  wgt_dfC <- data.frame(wgt_tblC)
  ans_C[i,] <- wgt_dfC[,3]
  ul_C[i,] <- wgt_dfC[,5]
  wgt_df <- data.frame(svymean(~voted, design=mydesignF))
  ans[i] <- wgt_df[,1]
  ul[i] <- wgt_df[,2]
}
colMeans(ans_R)
