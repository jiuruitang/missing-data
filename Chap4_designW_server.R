...
# adjust weights and inclusion probabilitiest
...
# only adjust unit nonrespondents weights to match N
sub_dat[which(sub_dat$U == 1),]$W <- (N_pop - sum(sub_dat$W[which(sub_dat$U == 0)]))/sum(sub_dat$U)
sub_dat$pi <- 1/sub_dat$W
...
save(resultListANWC,resultListMAR,sub_dat,MI_dataANWC,MI_dataMAR,pop_sd_HT,file = paste("./Chp4/pop4_designW/Mis_",run_index,".RData",sep=""))
